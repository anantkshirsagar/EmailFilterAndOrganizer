...
public class LabelFilterDBService extends AbstractDBService {
	...
	public void saveLabelFilter(FilterWrapper filterWrapper, String userEmailId)
		...
		String query = "insert into label_filter(label_id, is_email_id_filter, email_id_keywords, is_subject_filter, subject_keywords, is_body_filter, body_keywords, creation_date, filter_name) values(?,?,?,?,?,?,?,?,?)";
	...
	public List<LabelFilterGridWrapper> getLabelFilterGridInfo(String userEmailId, String filterNameSearchStr)
			throws ClassNotFoundException, SQLException, IOException {
		connectionSettings.build();
		...
		if (AppUtils.isStringNotEmpty(filterNameSearchStr)) {
			...
			query = "select filter.id, labelMetadata.name, filter.filter_name, filter.creation_date,"
					...
					+ "join label_filter filter on labelMetadata.id = filter.label_id "
		...
		LOG.debug("labelFilterGridInfoList {}", labelFilterGridInfoList);
		connectionSettings.closeConnection();
		return labelFilterGridInfoList;
	}

	public void updateLabelFilter(FilterWrapper filterWrapper, String userEmailId)
			throws ClassNotFoundException, SQLException, IOException {
		connectionSettings.build();
		String query = "update label_filter set label_id = ?, is_email_id_filter = ?, email_id_keywords = ?, is_subject_filter = ?, subject_keywords = ?, is_body_filter = ?, body_keywords = ?, creation_date = ?, filter_name = ? where id = ?";
		Timestamp creationTimestamp = new Timestamp(new Date().getTime());
		PreparedStatement prepareStatement = connectionSettings.getConnection().prepareStatement(query);
	...
	public FilterWrapper getLabelFilterByFilterName(String filterName, String userEmailId)
			throws SQLException, ClassNotFoundException, IOException {
		connectionSettings.build();
		String query = "select labelFilter.id, labelFilter.filter_name, labelFilter.label_id, "
				+ "labelFilter.is_email_id_filter, labelFilter.email_id_keywords, "
				+ "labelFilter.is_subject_filter, labelFilter.subject_keywords, labelFilter.is_body_filter, "
				+ "labelFilter.body_keywords, labelFilter.creation_date from label_filter labelFilter "
				+ "join core_label_metadata label on label.id = labelFilter.label_id "
				+ "join core_user_profile userProfile on userProfile.label_id = label.id "
				+ "where userProfile.user_email_id = ? and labelFilter.filter_name = ?";
		PreparedStatement prepareStatement = connectionSettings.getConnection().prepareStatement(query);
		prepareStatement.setString(1, userEmailId);
	...
	public void deleteFilter(int filterId) throws SQLException, ClassNotFoundException {
		connectionSettings.build();
		String query = "delete from label_filter where id = ?";
