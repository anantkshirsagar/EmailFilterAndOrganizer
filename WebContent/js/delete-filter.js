...
function labelFilterService() {
	...
	xhttp.setRequestHeader("userEmailId", sessionStorage.getItem("email"));
	xhttp.setRequestHeader("callType", "SAVE_DELETE_FILTER");
...
function getLabelFilterGridInfoService(searchFilter){
	...
	xhttp.setRequestHeader("userEmailId", sessionStorage.getItem("email"));
	xhttp.setRequestHeader("callType", "DELETE_FILTER_GRID_INFO_SERVICE");
	xhttp.send();
}
...
function edit(selectedFilterData){
	...
	openDialog("editDeleteFilterModal");
}
...
function editLabelFilterService(){
	...
	xhttp.onreadystatechange = function() {
		if (this.readyState == 4 && this.status == 200) {
			closeDialog("editDeleteFilterModal");
			getLabelFilterGridInfoService(null)
		}
	};
	...
	xhttp.open("POST", "DeleteFilterServlet", true);
	xhttp.setRequestHeader("userEmailId", sessionStorage.getItem("email"));
	xhttp.setRequestHeader("callType", "EDIT_DELETE_FILTER");
